  ...
  <div class="p-2">
    <transition :enter-active-class="proxy?.animate.searchAnimate.enter" :leave-active-class="proxy?.animate.searchAnimate.leave">
      <div class="search" v-show="showSearch">
        <el-form :model="queryParams" ref="queryFormRef" :inline="true" label-width="68px">
          ...
          <el-form-item label="指导老师" prop="competitionTeacherId">
            <el-select
              v-model="queryParams.competitionTeacherId"
              clearable
              filterable
              remote
              reserve-keyword
    ...
    <el-card shadow="never">
      <template #header>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['business:myCompetition:add']"> 新增 </el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()" v-hasPermi="['business:myCompetition:edit']"
              >修改
            </el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()" v-hasPermi="['business:myCompetition:remove']"
              >删除
            </el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['business:myCompetition:export']">导出 </el-button>
          </el-col>
        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['business:myCompetition:edit']"></el-button>
            </el-tooltip>
            <el-tooltip content="删除" placement="top">
              <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['business:myCompetition:remove']"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog :title="dialog.title" v-model="dialog.visible" width="500px" append-to-body>
      <el-form ref="competitionFormRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="竞赛类型" prop="competitionTypeId">
          <el-select v-model="form.competitionTypeId" placeholder="请选择竞赛类型" clearable style="width: 100%;">
            <el-option v-for="dict in types" :key="dict.competitionTypeId" :label="dict.competitionTypeName" :value="dict.competitionTypeId" />
          </el-select>
        </el-form-item>
        <el-form-item label="竞赛名称" prop="competitionName">
          <el-input v-model="form.competitionName" placeholder="请输入内容" />
        </el-form-item>
        <el-form-item label="指导老师" prop="competitionTeacherId">
          <el-select
            v-model="form.competitionTeacherId"
            clearable
            filterable
            remote
            reserve-keyword
        ...
        <el-form-item label="其他成员" prop="competitionOtherStudentIds">
          <el-select
            v-model="form.competitionOtherStudentIds"
            clearable
            multiple
            filterable
            remote
        ...
        <el-form-item label="其他老师" prop="competitionOtherTeacherIds">
          <el-select
            v-model="form.competitionOtherTeacherIds"
            clearable
            multiple
            filterable
            remote
        ...
        <el-form-item label="佐证材料" prop="competitionEvidenceUrl">
          <el-upload
            ref="upload"
...
import {
  ...
  updateCompetition,
  uploadFile
} from '@/api/business/myCompetition';
import {CompetitionVO, CompetitionQuery, CompetitionForm} from '@/api/business/myCompetition/types';
...
const handleUpdate = (row?: CompetitionVO) => {
  ...
  nextTick(async () => {
    ...
    res.data.competitionOtherTeacherIds = '';
    res.data.competitionOtherStudentIds = '';
    res.data.competitionTeacherId = '';
    res.data.competitionEvidenceUrl = '';
    loading.value = false;
  ...
  competitionFormRef.value?.validate(async (valid: boolean) => {
    if (valid) {
      ...
      if (form.value.competitionId) {
        ...
        await addCompetition(data).finally(() => buttonLoading.value = false);
      }
...
const handleDelete = async (row?: CompetitionVO) => {
  ...
  await proxy?.$modal.confirm('是否确认删除在校竞赛管理编号为"' + _competitionIds + '"的数据项？').finally(() => loading.value = false);
...
/** 导出按钮操作 */
const handleExport = () => {
  proxy?.download('business/competition/my/export', {
    ...queryParams.value
...
const handleUploadFile = async (options: UploadRequestOptions) => {
  ...
  form.value.competitionEvidenceUrl = url.msg
}
